...
# -----------------------------------------------------------------------------
# 4. Feature categories by data type
# -----------------------------------------------------------------------------
...
p_categories <- ggplot(
  feature_categories,
  aes(x = reorder(type, n_features), y = n_features, fill = type)
) +
  geom_col(show.legend = FALSE, width = 0.7) +
  geom_text(aes(label = n_features), hjust = -0.2, size = 4) +
  coord_flip() +
  labs(
    title = "Feature Categories by Data Type",
    x = NULL,
...
print(p_categories)
ggsave(
  file.path(viz_dir, "feature_categories_by_type.png"),
  p_categories,
  width = 8,
  height = 5
)
